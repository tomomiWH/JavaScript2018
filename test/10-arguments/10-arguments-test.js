...
const { destroyer } = require('../../exercises/10-arguments/argument');

describe('destroy', () => {
  it('should return [1, 1]', () => {
    const result = destroyer([1, 2, 3, 1, 2, 3], 2, 3);
    console.log(result);
    expect(result).to.be.equalTo([1, 1]);
  });
  it('should return [1, 5, 1]', () => {
    const result = destroyer([1, 2, 3, 5, 1, 2, 3], 2, 3);
    expect(result).to.be.equalTo([1, 5, 1]);
  });
  ...
  it('it should return hamburger', () => {
    ...
    expect(result).to.be.equalTo(['hamburger']);
  });
});
